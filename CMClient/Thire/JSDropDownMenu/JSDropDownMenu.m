...
- (void)animateLeftTableView:(UITableView *)leftTableView rightTableView:(UITableView *)rightTableView show:(BOOL)show complete:(void(^)())complete {
    ...
    if (show) {
        ...
        if (leftTableView) {
            ...
            // 计算下拉可用高度，适用于 - 状态栏 - 导航栏 - UITabBar
            CGFloat dropAllHeight = [UIScreen mainScreen] .bounds.size.height - CGRectGetHeight(self.frame) - 20;
            ...
            leftTableViewHeight = ([leftTableView numberOfRowsInSection:0] * leftTableView.rowHeight > dropAllHeight) ? dropAllHeight : ([leftTableView numberOfRowsInSection:0] * leftTableView.rowHeight);
